...
To switch to "read-write + persistent" mode, add this header to all requests `x-dogs-api-key: <key>`. For Formidables, the key for each environment (sandbox, development, staging, production) is stored in 1Password for aws-lambda-dogs.

A new key can be generated by running the command `openssl rand -hex 16`.

The key needs to be provided during deployment either as the `apikey` option to serverless, or in the API_KEY_SECRET environment variable.

```sh
STAGE=sandbox API_KEY_SECRET=<sandbox key> aws-vault exec FIRST.LAST-developer --no-session -- \
  yarn lambda:deploy
```
